...
|---+---------+---------------+---------------------------------------------|
|   |   <c>   |      <c>      |                                             |
| f |   arg   |    default    | description                                 |
|---+---------+---------------+---------------------------------------------|
| e |  code   |               | code to interpret                           |
...
#+CAPTION: awk-like, reads from stdin a field, and formats date on it to epoch
#+begin_src sh
  ruby -r date -F'\|' -ane 'puts DateTime.parse($F[0]).strftime("%s")' <<<"Sun Aug 31 13:04:02 2025|2820"
#+end_src
...
- inline gem dependencies on single ruby script file
- only intended for system gems (no vendor/local installs) https://github.com/rubygems/bundler/issues/5063#issuecomment-274400640
- but can be worked around by setting GEM_HOME https://github.com/rubygems/bundler/pull/7154
  #+begin_src sh
    GEM_HOME=vendor/bundle ruby3.0 foo.rb
  #+end_src
- or as the first line of the script
  #+begin_src ruby
