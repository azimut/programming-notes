...
- gs
  - -g device width/height
  - -r device x/y resolution
  - convert to =.png=
    #+begin_src sh
      gs -sDEVICE=pngalpha -sOutputFile=output.png -dBATCH -dNOPAUSE foo.ps -c showpage
    #+end_src
  - systemdict setting
    #+begin_src sh
      -dname=token  # sets systemdict
      -sname=string # sets systemdict
    #+end_src
