...
| signal.h |  sigaction   | Signal handling                                                      |
...
|-----+---------+---------------------------------------|
| <r> |   <c>   |                                       |
|   j |   "-"   | justification, "-" for right          |
|   f | "0" " " | fill with                             |
|   s |   "+"   | sign, always prints sign symbol       |
|   w |    N    | width, reserve N chars for number     |
|  .p |    N    | precision, N digits after decimal     |
|   t | "f" "d" | type                                  |
...
#+begin_src c
  #if 0
  gcc  -Wall -Wextra -pedantic -O3 $0 -o out -lm && ./out
  exit 0
  #endif
#+end_src
...
unlike *fgets*, *getline* dynamically resized the buffer read.
...
see ~ip.7~
|------------+----------+-------------+---|
| ipv4 + tcp | AF_INET  | SOCK_STREAM | 0 |
| ipv6 + tcp | AF_INET6 | SOCK_STREAM | 0 |
| ipv4 + udp | AF_INET  | SOCK_DGRAM  | 0 |
| ipv6 + udp | AF_INET6 | SOCK_DGRAM  | 0 |
|------------+----------+-------------+---|
*** setsockopt/5
see ~socket.7~, ~tcp.7~, ~ip.7~, or ~unix.7~
|---------+---------------------------------------------------|
| sockfd  |                                                   |
| level   | use SOL_SOCKET, or /etc/protocols number          |
| optname | eg: SO_REUSEADDR                                  |
| *optval | different options use different types, eg: ~&val~ |
| optlen  | sizeof(val)                                       |
|---------+---------------------------------------------------|
*** fcntl/3
...
O_NONBLOCK, only accepted settable flag value for sockets

*** bind/3

fd sockaddr* socklen_t

host(le) to network(be) order: htons/1 htonl/1

#+begin_src c
  struct sockaddr_in addr = {
      .sin_family = AF_INET,
      .sin_port = htons(1234),
      .sin_addr.s_addr = htonl(0), // 0.0.0.0
  };
#+end_src

*** accept/3

blocks, consumes an item from a queue
non-blocking, has a readiness notificatin
...
- under normal conditions, they can return less than the requested number of bytes (on sockets)
- is a good idea to write wrappers that ensure all needed is read/write
- they also might errno (-1) momentarilly, eg: EINTR, or EAGAIN for non-blocking versions

|----------+----------+---------------------------------------|
| read     | write    | with a single continuos buffeer       |
| readv    | writev   | with multiple buffers                 |
| recv     | send     | +flag                                 |
| recvfrom | sendto   | +remote sockaddr                      |
| recvmsg  | sendmsg  | with multiple buffers +flags          |
| recvmmsg | sendmmsg | multiple recvmsg/sendmsg in 1 syscall |
|----------+----------+---------------------------------------|

- *read()* merely copies data from the kernel-side buffer, and when the buffer is empty, read() suspends the calling thread until more data is ready
- *write()* merely puts the data into a kernel-side buffer for the TCP stack to consume, and when the buffer is full, write() suspends the calling thread until there is room

*** readiness APIs
for sockets, pipes, signalfd, not files
1) select - hard limit to 1024 fds
2) epoll_wait - fd list is stored in the kernel (through epoll_ctl)
3) kqueue - bsd, like epoll, requires fewer syscalls
4) io_uring - ????
**** poll/3

(struct pollfd*) nfds_t timeout

takes an array of pollfd

#+begin_src c
  struct pollfd {
    int   fd;
    short events; // provided flag: POLLIN|POLLOUT
    short revents;// returned flag, indicates readines
  };
#+end_src

*** other
|-------------+----+-----------------------|
| getsockname | /3 | get fd local address  |
| getpeername | /3 | get fd remote address |
| getaddrinfo | /4 | name resolution       |
|-------------+----+-----------------------|
** qsort
...
avoid fork defunct process

#+begin_src c
  void signal_handler(__attribute__((unused)) int s) {
