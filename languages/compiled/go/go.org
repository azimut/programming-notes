...
- Receiver Types: Value, Pointer
- Mixing receiver types, on the same object type is frowned upon.
...
- to have multiple "package main" .go files in the same dir, add this to the top
  #+begin_src go
    //go:build ignore
    package main
  #+end_src
