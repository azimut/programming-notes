...
- 1976, at Bell Labs
- by Stuart Feldman
- part of the [[https://pubs.opengroup.org/onlinepubs/9699919799/utilities/make.html][POSIX]] standard, optional
...
#+begin_src makefile
  define common-png-steps
  	@echo "--- Running common step for $(@): Verifying dimensions ---"
  endef
...
- no support for filenames with:
  - [[https://stackoverflow.com/questions/9838384/can-gnu-make-handle-filenames-with-spaces][with spaces]]
  - with ~:~, must be escaped (aka ~"multiple target patterns.  Stop."~ error)
- no support for *arithmetic operations* (use bash's arithmetic evaluation)
- no support for *boolean operations* on *ifeq* (can be hacked w/filter)
...
- Target order does NOT matter, is from more precise to less one.
...
- to do Process Substitution we need to:
  - use *bash -c*
  - escape *$* with *$* and *\* (one for make other for bash itself)
