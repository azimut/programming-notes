- Welcome to Elm! by Ryan Haskell https://www.youtube.com/playlist?list=PLuGpJqnV9DXq_ItwwUoJOGk_uCr72Yvzb
...
We need to get a Request and then convert it to a Cmd
...
|------------------------------+----------------------------------------------------------------------------------|
| source                       | https://github.com/rtfeldman/elm-0.19-workshop                                   |
...
- it is common to just return it
  - when we don't want to return anything on a case text ""
...
- instead, you can export things that will keep the invariant
  #+begin_src elm
module Email exposing (Email, fromString, toString) -- THIS enforces the invariant
...
- the opaque type might not even have any data to enforce the invariant
  #+begin_src elm
    type Email = Email
  #+end_src
...
#+CAPTION: snippet from rtfeldman/elm-validate
#+begin_src elm
...
Given this, we can create our own *submitForm*

#+begin_src elm
  submitForm : Valid Form -> Http.Request User
#+end_src

- The only way to *submitForm* is with a *Valid* type.
- And the only way to get *Valid* it is with a call to *validate*
- Makes the *invariant* enforceable by the types and the compiler
...
- Split into modules
  - Such as each module fits in your head
  - Every page has his own Model
...
Adding arguments to show business dependencies logic
...
- a Route is an abstraction on top of valid urls
- Route a custom type enum
- each url will map to a particular Route variant
...
parser =
    oneOf -- runs 1 at the time, returns successful
...
Helper to generate links from a Route.
Such as nothing outside this module should be dealing with urls directly.
...
- a =Page= is a non-opaque custom enum type
- not necesarilly correspond with ~urls~
  - they correspond with displays
...
view maybeViewer page { title, content } =
    { title = title ++ " - Conduit"
    , body = viewHeader page maybeViewer :: content :: [ viewFoter ]
    }
#+end_src
...
- keep it as flat as possible
  - this leads to exporting less (?
  - no Data/, no Request/

- this example uses folders for use as namespaces

- against using type alias

*** Loading & Persisting Data

- loading
  - render all possible while waiting for other things
    - maybe localized spinner
  - block all page until all is ready
    - render a spinner
