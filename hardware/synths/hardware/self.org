...
- Can be (mostly between 0v and 12v)
  - Gates
  - Triggers
...
- CV
  - frequency
    - exponential CV (1V/1oct)
    - linear CV (vibrato)
  - pulse width
  - sync
    - from another VCO, reset after sync finish a cycle.
...
- Other waves:
  - sawtooth |\
  - square: may allow duty cycle control
  - rectangle or pulse: not 50% duty cycle
  - sine
  - triangle

** VCF = Voltage Controlled Filter

- maybe include a mixer
- Inputs
  - (key) exponential CV (1v/1oct)
    - cutoff freq or resonance freq
    - 1 control per filter
  - (key) linear CV
  - (Audio) Signal IN
- Outputs: LPF/HPF/BPF
- CV manual controls
  - resonance (frequency) ammount
    - adds emphasis to center freq, results in "ringing"
