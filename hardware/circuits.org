...
- circuit design
  - logic inputs should be on left or up
  - logic outputs should be on right or down
...
- Electronic CAD libraries http://smisioto.no-ip.org/elettronica/kicad/kicad-en.htm
- Course
...
- helps with impedance problems
- transistor directly (*collector/emitter*) connected to power rails
- Vin comes at the *base*
- gain of 1 for voltage: Vload/Vin ~= 1
- amplifies current: Iload/Iin = B + 1 = B
  - B current comming in
  - B = 100

** Differential Pair/Amplifier (aka long tailed pair)

  - 2 transistors
  - individual ICs have variations, but 2 of the same match very wel
  - taking advantage of the matching, you get better performance (?)
    - amplifying the difference, and ignoring the common
  - https://www.allaboutcircuits.com/textbook/designing-analog-chips/differential-pairs/introduction-to-differential-pairs/
