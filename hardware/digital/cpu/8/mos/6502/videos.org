...
- we can reshape the CLOCK signal
  - to capture the whole event with a single pulse
  - with a resistor, diode, capacitor, NOT gate with schmitt trigger
  - carefully to avoid noise that might be interpreted as interrupts by the NOT gate
...
#+CAPTION: using a circular buffer to process keys
#+begin_src asm
