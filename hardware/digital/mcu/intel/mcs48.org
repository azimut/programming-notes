...
- 8 bit
- 96 instructions
  - 1 byte are those that only have the opcode (90%?) (aka operate)
...
- From LSB to MSB
  - 3 bit stack pointer
  - 1 bit unused
  - 1 bit memory bank select switch
    #+begin_src asm
      sel rb0 ; = 0 for R0  - R7
      sel rb1 ; = 1 for R0' - R7'
    #+end_src
    ...
    - =Register Inderect Addressing=
      - R0/R1 may be used as *RAM pointers* to access memory containing data.
        #+begin_src asm
          mov a, @r0 ; a = *r0
        #+end_src
  - (8-23) 8-Level stack (or used as user ram if not stack)
    - 16x8
