...
| BASH_SOURCE  |           | array                              |
| FUNCNAME     |           | array, functions on stack          |
| LINENO       | 10        | current line number                |
...
- reserved words
- shell builtin (see "help" command)
...
** shopt
- https://gist.github.com/detain/dfa0e4d75647b424c5aea45a34af0713
...
- * Matches any string, including the null string.  When the globstar shell option is enabled, and * is used in  a  pathname
  expansion context, two adjacent *s used as a single pattern will match all files and zero or more directories and subdi‐
  rectories.  If followed by a /, two adjacent *s will match only directories and subdirectories.
