...
movement <- velocity <- acceleration <- force
...
- Custom distribution of random numbers
  - aka non-uniform/non-gaussian distribution
  - WHY? to avoid hitting the same spots (aka ~oversampling~)
  - Solutions
    1) =Levi Flight= take a big step every so often
       - the longer the steps, the LESS likely to take it
       - the smaller the step, the MORE likely to take it
    2) =accept-reject algorith=
       - a type of *monte carlo* method
       - eg: wastful example
         1. on a infinite loop
         2. get 2 random numbers
            - a random nr
            - a ~qualifying random number~
         3. if 2nd<1st
         4. return 1st
         5. else goto 2.
...
- is the ~force~ of gravity on an object (in newtons)
- an object has same mass on earth and moon
- weight = mass x gravity's_acceleration

**** mass

- measure of ammount of ~matter~ in an object (in kg)
- an object has same mass on earth and moon
- mass != weight

**** density

- the ammount of ~mass~ per unit of volume (in g in cm3)

*** Newton's 3 laws of motion
**** Inertia

- An object at rest stays at rest, and an object in motion stays in motion, at constant speed and direction unless acted upon by an unbalanced ~force~.

- alt: An object's ~velocity~ vector will remain constant if it's in a state of equilibrium.
...
- For every action, there is an equal and opposite reaction.
   - alt: Forces always occur in pairs. Both of equal strength but in opposite directions.
   - alt: If you calculate a ~p5.Vector~ called ~f~ that represents a force of object A and object B, you must also apply the opposite force that object B exerts on object A. You can calculate this other foce as ~p5.Vector.mult(f,-1)~
   - aka resistance is measure as a force

*** Code

 - accumulate forces to be added together
 - acceleration should have no memory, aka be set to 0 on each cycle
   - in a time-based physics simulation
 - ideally you would have a constant concept of time, expressed on ~dt~
